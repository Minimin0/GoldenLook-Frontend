...
심사에서 로그인 단계를 없애기 위해 `lib/demo-account.ts` 의 `DEMO_AUTO_LOGIN` 이 켜져 있습니다.
세션이 없으면 `AuthProvider` 가 공용 계정으로 자동 로그인하고, 홈·헤더의 로그인 입구를 감춥니다.

- `/login` 과 `RequireAuth` 는 지우지 않았습니다. `DEMO_AUTO_LOGIN = false` 로 되돌리면 원래 로그인 흐름이 그대로 살아납니다.
- 계정 정보는 브라우저 번들에 들어갑니다. 공개된 계정으로 취급하고, 그 비밀번호를 다른 곳에 쓰지 않습니다.
- 모든 방문자가 같은 계정을 쓰므로 `/my` 목록이 공유됩니다. 실제 인물 사진·실제 연락처를 넣지 않습니다.
- 401 이 나면 세션을 버리고 다시 자동 로그인합니다. 재시도는 페이지당 3회까지이고, 그래도 실패하면 빈 화면 대신 "다시 시도" 안내를 보여 줍니다.
