...
> **제출용 데모 자동 로그인 (한시적)**
> `lib/demo-account.ts` 의 `DEMO_AUTO_LOGIN` 이 켜져 있어 방문자는 공용 계정으로 자동 로그인된 상태로 들어옵니다.
> 로그인·회원가입 입구는 화면에서 감췄고, `/login` 자체는 그대로 남아 있습니다.
> 플래그를 `false` 로 되돌리면 원래 로그인 흐름이 복구됩니다. 자세한 주의사항은 `AGENTS.md` 를 참고하세요.
